...
#define TCP_SOFT_ACK            0     /* soft ack enabled */
#define TCP_ORD_PRINT           1
#if TCP_ORD_PRINT==0
# define printf(arg...)
#endif
...
static void TcpOrdPrint(order *ord) {
	...
	while (put != NULL) {
		if (nser != 0 && nser != put->seq)
			printf("***->");
		printf("%lu(%lu-%d-%d-%lu)->", put->seq, put->nxt_seq, put->cng,
				put->ack, put->pkt->len);
		if (put->seq != 0 && put->nxt_seq - put->seq != 1 && put->nxt_seq
				- put->seq != put->pkt->len) {
			printf("\n");
			LogPrintf(LV_OOPS, "TCP internal tcp length error pkt:%lu len:%lu",
					put->nxt_seq - put->seq, put->pkt->len);
			ProtStackFrmDisp(put->pkt->stk, TRUE);
			exit(-1);
		}
		if (put->pkt->len > put->pkt->raw_len && put->pkt->raw != NULL) {
			LogPrintf(LV_OOPS, "TCP data dimension error raw:%lu data:%lu",
					put->pkt->raw_len, put->pkt->len);
			ProtStackFrmDisp(put->pkt->stk, TRUE);
			exit(-1);
		}
		nser = put->nxt_seq;
...
static int TcpFlush(int flow_id, order *ord, bool mono) {
	...
	cnt = 0;
	while ((ord->src != NULL || ord->dst != NULL) && (mono == FALSE
			|| (ord->num > TCP_SUBDIS_MONO_PKT_MARG))) {
		cnt++;
		if (ord->src_put) {
			if (ord->src != NULL && ord->seq_s == ord->src->seq) {
				put = ord->src;
...
static int TcpOrder(int flow_id, order *ord, packet *pkt, unsigned long seq,
		unsigned long nxt_seq, bool src) {
...
static int TcpSyn(int flow_id, order *ord, packet *pkt, unsigned long seq,
		unsigned long nxt_seq, bool src) {
	struct seq *ins;
...
static int TcpFin(int flow_id, order *ord, packet *pkt, unsigned long seq,
		unsigned long nxt_seq, bool src) {
	if (src == TRUE)
		ord->fin_s = seq;
	else
...
static int TcpRst(int flow_id, order *ord, packet *pkt, unsigned long seq,
		unsigned long nxt_seq, bool src) {
	ord->rst = TRUE;
...
static void TcpSubDissector(int flow_id, packet *pkt) {
	...
	if (ord->mono == TRUE && ord->num > (TCP_SUBDIS_MONO_PKT_LIMIT
			+ TCP_SUBDIS_MONO_PKT_MARG)) {
...
static packet* TcpDissector(packet *pkt) {
	...
	if (pkt->len < sizeof(struct tcphdr)) {
		LogPrintf(LV_WARNING,
				"TCP header packet length error (tcp:%i pkt:%i tcp_header:%i)",
				sizeof(struct tcphdr), pkt->len, sizeof(struct tcphdr));
		ProtStackFrmDisp(pkt->stk, TRUE);
		PktFree(pkt);
	...
	if (ProtFrameProtocol(pkt->stk) == ip_id) {
		...
		if (computed_cksum != 0) {
			LogPrintf(LV_WARNING, "TCP packet chechsum error 0x%x",
					computed_cksum);
			//ProtStackFrmDisp(pkt->stk, TRUE);
			PktFree(pkt);
...
int DissectFlowHash(cmpflow *fd, unsigned long *hashs) {
	...
	if (ProtFrameProtocol(ip) == ip_id) {
		...
		if (o->ip_mx.uint32 < o->ip_mn.uint32 || (o->ip_mx.uint32
				== o->ip_mn.uint32 && o->port_mx < o->port_mn)) {
			tmp = o->ip_mn;
			o->ip_mn = o->ip_mx;
			o->ip_mx = tmp;
